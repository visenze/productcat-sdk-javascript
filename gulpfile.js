...
gulp.task('webpack', () => gulp.src(['./js/productcat.js']) // gulp looks for all source files under specified SETTINGS
...
gulp.task('webpack-production', () => gulp.src('./js/productcat.js') // gulp looks for all source files under specified SETTINGS
  .pipe(stream(webpackConfig)) // blend in the webpack config into the source files
  .pipe(replace({ patterns: [{ match: 'version', replacement: version }] })) // replace some variables in source
...
if (awsProfile && awsProfile.params && awsProfile.params.Bucket) {
  ...
  gulp.task('deploy', () => gulp.src(`dist/js/productcat-${version}.min.js`)
    .pipe(rename((path) => {
      // upload folder name
      path.dirname = '/productcat/dist/js/';
    }))
...
gulp.task('watch', () => {
  gulp.watch('./js/productcat.js', gulp.series('webpack'));
